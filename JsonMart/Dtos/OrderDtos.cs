...
public record OrderDto(int Id, DateTime OrderDate, int? UserId, List<OrderProductDto>? Products, OrderStatus Status, decimal TotalPrice);
public record OrderCreateDto(int UserId, List<int> ProductIds);
public record OrderProductDto(int Id, string? ProductName, decimal? Price, string? Description, int Quantity);
